...
async def cmd_newgig(
    ...
    if len(args) < 2:
        await update.message.reply_text(
            _("Usage: /newgig Title | price_usd | description")
        )
        return
    payload = args[1]
...
async def cmd_newgigbtc(
    ...
    if len(args) < 2:
        await update.message.reply_text(
            _("Usage: /newgigbtc Title | price_btc | description")
        )
        return
    payload = args[1]
    parts = [p.strip() for p in payload.split("|")]
...
async def cmd_listings(
    ...
    _ = get_translation(user)
    gigs = (
        db.query(Gig).filter(Gig.active == True).order_by(Gig.id.desc()).limit(20).all()
    )
    if not gigs:
...
async def cmd_mygigs(
    ...
    for g in gigs:
        lines.append(
            f"#{g.id} — *{g.title}* — ${g.price_usd} — {g.currency} — {_('ON') if g.active else _('OFF')}"
        )
    await update.message.reply_text("\n".join(lines))
...
async def cmd_buy(
    ...
    await update.message.reply_text(
        _(
            "🛡️ Order created. Deposit *{amt}* in {asset} to the address:\n`{addr}`\n\n"
...
async def cmd_confirm_tx(
    ...
    await update.message.reply_text(
        _(
            "✅ Deposit confirmed. Funds are in escrow. Use /release {oid} when ready."
        ).format(oid=oid)
    )
...
async def cmd_release(
    ...
    db.refresh(o)
    await update.message.reply_text(
        _("🔓 Release marked. (On-chain transfer by worker/admin)")
    )


@db_session_decorator
async def cmd_dispute(
    update: Update, context: ContextTypes.DEFAULT_TYPE, db: Session = None
):
    user = await ensure_user(update.effective_user, db)
    _ = get_translation(user)
    ...
    if settings.ADMIN_USER_ID:
        try:
            await context.bot.send_message(
                ...
                text=_("⚖️ Dispute for order {oid}: {reason}").format(
...
async def cmd_orders(
    ...
    for o in orders:
        lines.append(
            f"#{o.id} — {o.status} — ${o.expected_amount} — {o.deposit_address or '-'}"
        )
    await update.message.reply_text("\n".join(lines))
