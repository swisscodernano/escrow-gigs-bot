...
def apply_deposit(db, user_id: int, amount: Decimal, ext_ref: str = "") -> Transaction:
    ...
    tx = Transaction(user_id=user_id, type="deposit", amount=amount, status="succeeded", ext_ref=ext_ref)
    db.add(tx)
    db.commit()
    db.refresh(tx)
    db.refresh(w)
...
def place_hold(db, user_id: int, amount: Decimal, ext_ref: str = "") -> Transaction:
    ...
    w.balance = Decimal(w.balance) - amount
    tx = Transaction(user_id=user_id, type="hold", amount=amount, status="succeeded", ext_ref=ext_ref)
    db.add(tx)
    db.commit()
    db.refresh(tx)
    db.refresh(w)
...
def release_hold(db, hold_tx_id: int, as_refund: bool = False) -> Transaction:
    ...
    if as_refund:
        ...
        tx_type = "payout"
    tx = Transaction(user_id=hold.user_id, type=tx_type, amount=hold.amount, status="succeeded", ext_ref=hold.ext_ref)
    db.add(tx)
    db.commit()
    db.refresh(tx)
    if as_refund:
