...
def process_withdrawal(order_id: int):
    ...
    try:
        ...
        source_address_ctx = wallet_ctx.Purpose().Coin().Account(0).Change(Bip44Changes.CHAIN_EXT).AddressIndex(0)
        ...
        print(f"[WORKER] Simulating transaction from {source_address} to {seller.btc_address} for {order.expected_amount} BTC.")
        ...
        db.commit()
        print(f"[WORKER] Withdrawal for order #{order.id} completed successfully (simulated). Transaction ID: {tx_hash}")
